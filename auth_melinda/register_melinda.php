<!DOCTYPE html>
<html>
<head>
    <title>Register Siswa</title>
</head>
<body>

<h2>Register Siswa</h2>

<form action="proses_register_melinda.php" method="POST">
    <label>Username</label><br>
    <input type="text" name="username_melinda" required><br><br>

    <label>Password</label><br>
    <input type="password" name="password_melinda" required><br><br>

    <label>Nama Lengkap</label><br>
    <input type="text" name="nama_lengkap_melinda" required><br><br>

    <label>NIS</label><br>
    <input type="text" name="nis_melinda" required><br><br>

    <label>Kelas</label><br>
    <input type="text" name="kelas_melinda" required><br><br>

    <button type="submit">Daftar</button>
</form>

<p>Sudah punya akun? <a href="login_melinda.php">Login</a></p>

</body>
</html>
